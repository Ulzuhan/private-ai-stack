...
# docker save/load loses RepoDigests metadata (classic image store), so a
# loaded image cannot be resolved by its name:tag@digest reference. The
# bundle's compose copy pins by tag — which the checksums below tie to the
# exact packaged bits — and MANIFEST.txt records the full pinned refs.
{
  echo "# Bundle copy generated by package-offline.sh: images pin by tag here"
  echo "# because docker save/load drops digest metadata. The full pinned"
  echo "# name:tag@digest references are recorded in MANIFEST.txt."
  sed -E 's/@sha256:[0-9a-f]{64}//g' "$ROOT/docker-compose.yml"
} >"$bundle_dir/compose/docker-compose.yml"
cp "$ROOT/docker-compose.airgap.yml" "$bundle_dir/compose/"
